...
(covering fixed-rate, intermittent, and target-specific variants) with HMAC-based packet authentication for low-overhead deployment on resource-constrained vehicular nodes. The full mode integrates a fine-tuned Large Language Model (LLM) with an RSU-maintained, blockchain-verified Federated Learning (FL) aggregator for semantic anomaly detection, combined with post-quantum cryptographic mitigation employing a controller-independent PQC Logical Key Hierarchy (PQC-LKH) with Distributed Key Generation, CRYSTALS-Dilithium signatures, zk-SNARK-based packet forwarding proofs, and $(k,n)$-threshold signatures for collectively authorized node isolation without controller key monopoly. A novel Mobility-Aware Trust Decay (MATD) mechanism and a Dual-Evidence Blockchain Smart Contract (DEBSC) ensure that RSU handoff ambiguity does not inflate false positive rates. 
...
unified under a mobility-aware trust mechanism and enforced by a post-quantum
cryptographic mitigation layer that employs zero-knowledge forwarding proofs,
threshold signatures, PQC-LKH group re-keying with DKG-based
controller-independent key generation, and CRYSTALS-family PQC — the first
such integrated framework providing controller-independent mitigation enforcement
for grey hole attack detection in SDVN.
...
$\boldsymbol{\theta}$ are the fine-tuned LLM parameters, and the 
malicious class probability is extracted from the softmax output. 
A higher $\mathcal{Q}_i(t)$ indicates greater likelihood of grey 
hole behaviour.
...
holds a Kyber-768 key pair $(\mathbf{pk}_u, \mathbf{sk}_u)$. The
...
\hline
Attack model & Grey hole; drop rate $50\%$ default; attacker \% swept $20$--$80\%$ & Sweeps the adversary range; $50\%$ drop is the canonical grey hole operating point. \\
\hline
...
to 68.71\%), jitter rises from 27.46 to 38.09, and detection accuracy degrades
from 92.39\% to 71.74\%. These preliminary results confirm that the ns-3.35
...
with the three external baselines (B1--B3) and five ablations (A1--A5) defined in
Section~\ref{sec:evaluation}. Since full simulation experiments are scheduled for
